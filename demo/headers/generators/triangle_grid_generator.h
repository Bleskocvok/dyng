...
/// Generates a growing triangular grid.
class triangle_grid_generator : public generator {
public:
    static std::unique_ptr<generator> parse(const std::vector<std::string>& args) {
        ...
        return std::make_unique<triangle_grid_generator>(
                std::stoi(args[2]));
    }

    triangle_grid_generator(unsigned steps) : generator(steps + 1) {}
    ...
    void next_step() override {
        ...
        for (unsigned i = 1; i < next_layer.size() - 1; ++i) {
            add_edge(prev_layer[i - 1], next_layer[i]);
        }
